...
void cv_jit_learn_read(t_cv_jit_learn *x, t_symbol *s, short argc, t_atom *argv)
{
	...
	{
		//Free storage
		freebytes(x->mean, x->size * sizeof(double));
		...
		x->inverse = (double *)getbytes(x->size * x->size * sizeof(double)); //Allocate memory for inverse matrix
		//Read index
		count = sizeof(double);
...
void *cv_jit_learn_new(long arg)
{
	...
	x->mean = (double *)getbytes(x->size * sizeof(double)); //Allocate memory for mean vector
	x->covariance = (double *)getbytes(mat_size * sizeof(double)); //Allocate memory for covariance matrix
	x->inverse = (double *)getbytes(mat_size * sizeof(double)); //Allocate memory for inverse matrix
